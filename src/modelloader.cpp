...
ModelData ModelLoader::getModelData(QString filePath)
{
    load3DModel(filePath);

    ModelData data;
    getBufferData(&data.vertices, &data.normals, &data.indices);
    getTextureData(&data.textureUV);

    data.rootNode = getNodeData();

    return data;
}

bool ModelLoader::load3DModel(QString filePath)
{
    ...
    for (auto item : m_textureUV) {
        item.clear();
    }
    m_textureUV.clear(); // multiple channels
    m_textureUVComponents.clear(); // multiple channels
