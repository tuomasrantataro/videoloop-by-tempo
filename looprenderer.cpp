...
static const int UNIFORM_DATA_SIZE = 16 * sizeof(float) + sizeof(uint);
...
LoopRenderer::LoopRenderer(QVulkanWindow *w)
    : m_window(w)
{
}

VkShaderModule LoopRenderer::createShader(const QString &name)
{
    QFile file(name);
...
bool LoopRenderer::createTextures()
{
    ...
    m_texFormat = VK_FORMAT_R8G8B8A8_UNORM;
    ...
    if (canSampleLinear && !alwaysStage) {
        ...
        if (!writeLinearImages(images, m_texImage, m_texMem))
            return false;
        ...
        if (!writeLinearImages(images, m_texStaging, m_texStagingMem))
            return false;
    ...
    viewInfo.subresourceRange.layerCount = frameCount;
...
bool LoopRenderer::createTextureImage(const QSize &size, VkImage *image, VkDeviceMemory *mem,
                                        VkImageTiling tiling, VkImageUsageFlags usage, uint32_t memIndex)
{
    ...
    imageInfo.arrayLayers = frameCount;
...
bool LoopRenderer::writeLinearImages(const QList<QImage> &images, VkImage image, VkDeviceMemory memory)
{
    VkDevice dev = m_window->device();
...
void LoopRenderer::ensureTexture()
{
    ...
    barrier.subresourceRange.layerCount = frameCount;
    ...
    if (m_texLayoutPending) {
        ...
        copyInfo.srcSubresource.layerCount = frameCount;
        copyInfo.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        copyInfo.dstSubresource.layerCount = frameCount;
        // self-added base layers
        //copyInfo.dstSubresource.baseArrayLayer = 1;
        copyInfo.srcSubresource.baseArrayLayer = 1;
...
void LoopRenderer::initResources()
{
    ...
    for (int i = 0; i < concurrentFrameCount; ++i) {
        ...
        // own addition
        memcpy(p + offset + 16*sizeof(float), &currentIndex, sizeof(uint));
...
void LoopRenderer::initSwapChainResources()
{
...
void LoopRenderer::releaseSwapChainResources()
{
    qDebug("releaseSwapChainResources");
}

void LoopRenderer::setBpm(float f) {
    qWarning("setting tempo to %.1f bpm", f);
    m_bpm = f;
    frameCounter = 0;
    droppedFrames = 0;
    addedFrames = 0;
...
void LoopRenderer::releaseResources()
{
...
void LoopRenderer::startNextFrame()
{
    VkDevice dev = m_window->device();
    VkCommandBuffer cb = m_window->currentCommandBuffer();
    const QSize sz = m_window->swapChainImageSize();
    ...
    if (err != VK_SUCCESS)
        qFatal("Failed to map memory: %d", err);
    memcpy(p, m_proj.constData(), 16 * sizeof(float));
    memcpy(p + 16*sizeof(float), &frameIndexes[currentIndex], sizeof(uint));
    m_devFuncs->vkUnmapMemory(dev, m_bufMem);

    // Calculate whether to skip or double frames to maintain wanted framerate
    auto refreshRate = m_window->screen()->refreshRate();
    double framesPerBeat = double(refreshRate)*60.0/m_bpm;

    double frameCountDifference = frameCount - framesPerBeat;

    double addFrequency = framesPerBeat/frameCount;
    double dropFrequency = frameCountDifference/frameCount;

    if (frameCountDifference < 0) {
        // If we need to slow down replay loop
        double currentFrequency = double(frameCounter+addedFrames)/double(frameCounter);
        if (currentFrequency < addFrequency) {
            addedFrames++;
        } else {
            frameCounter++;
            currentIndex++;
        ...
        // if we need to speed up replay loop.
        frameCounter++;
        currentIndex++;
        double currentFrequency = double(droppedFrames)/double(frameCounter);

        while (currentFrequency < dropFrequency) {
            droppedFrames++;
            currentIndex++;
            frameCounter++;
            currentFrequency = double(droppedFrames)/double(frameCounter);
        }
    }
    ...
    std::chrono::duration<double> elapsed = end-prevFrame;
    prevFrame = end;
    float time = 1/elapsed.count();
    float alpha = 0.01;
    fpsAverage = (alpha * time) + (1.0-alpha) * fpsAverage;
    //qWarning("framerate: %f fps, average %f fps", 1/elapsed.count(), fpsAverage);
